        ...
        try:
            ...
            if match:
                ...
                formatted_output = "\n".join(
                    [
                        "    " + line
                        for line in args.output.replace("\\n", "\n").splitlines()
                    ]
                )
                ...
                if (
                    "测试跨模型工具" in header
                    or "给我写个记录工具" in prompt_str
                    or "```markdown" in formatted_output
                    or "无明显输出内容" in formatted_output
                    or "待总结的AI回复内容如下" in formatted_output
                ):
...
def cmd_record(args):
    ...
    formatted_prompt = format_prompt(args.prompt)
    formatted_output = "\n".join(
        ["    " + line for line in args.output.replace("\\n", "\n").splitlines()]
    )
